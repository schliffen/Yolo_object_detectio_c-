...
const int  nms(const std::vector<std::vector<float>>  inboxes, const float  confThreshold, const float  nmsThreshold, const int classId, std::vector<float>  inConfidences, 
      std::vector<size_t> & outnmsClassIds, std::vector<float> & outConfidences, std::vector<std::vector<float>> & outboxes)
{
  if (inboxes.empty()){
    std::cout<< "there is no boxes! \n";
    outboxes = std::vector<std::vector<float>>();
    outConfidences = std::vector<float>();
    return 0;
  }
  	// return std::vector<std::vector<float>>();//  vector<Rect>();
  
  // grab the coordinates of the bounding boxes
  auto x1 = GetPointFromRect(inboxes, XMIN);
  auto y1 = GetPointFromRect(inboxes, YMIN);
  auto x2 = GetPointFromRect(inboxes, XMAX);
  auto y2 = GetPointFromRect(inboxes, YMAX);
  ...
  while (idxs.size() > 0) {
    ...
    auto deleteIdxs = WhereLarger(overlap, nmsThreshold);
    deleteIdxs.push_back(last);
    idxs = RemoveByIndexes(idxs, deleteIdxs);
  };

  for (const auto & idx: pick){
    outboxes.push_back( inboxes[idx] );
    outConfidences.push_back( inConfidences[idx] );
    outnmsClassIds.push_back( classId );
  }
  return 1;
  // return FilterVector(inboxes, pick); // using arrays instead of rectangles
}
